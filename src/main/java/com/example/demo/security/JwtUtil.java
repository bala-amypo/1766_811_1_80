...
public class JwtUtil {
    ...
    // This is the missing method the compiler is complaining about!
    public String generateTokenForUser(UserAccount user) {
        Map<String, Object> claims = new HashMap<>();
        claims.put("email", user.getEmail());   // Matches t69
        claims.put("role", user.getRole());     // Matches t70
        claims.put("userId", user.getId());     // Matches t71
    ...
    // Required by AuthController.login to extract email after authentication
    public String extractUsername(String token) {
        return Jwts.parserBuilder()
                .setSigningKey(getSigningKey())
                .build()
